...
s2.mRF.train.df <- cbind(train.trait.df[,-c(8:10)],s2.train.spectr.20m.df) #we remove the fixed parameters
s2.mRF.valid.df <- cbind(valid.trait.df[,-c(8:10)],s2.valid.spectr.20m.df) #we remove the fixed parameters

#case of 7 params
N.train.df   <- s2.mRF.train.df[,-c(2:7)]
Cab.train.df <- s2.mRF.train.df[,-c(1,3:7)]
Car.train.df <- s2.mRF.train.df[,-c(1:2,4:7)]
Cw.train.df  <- s2.mRF.train.df[,-c(1:3,5:7)]
Cm.train.df  <- s2.mRF.train.df[,-c(1:4,6:7)]
LAI.train.df <- s2.mRF.train.df[,-c(1:5,7)]
...
st.test.df <- valid.trait.df[,-c(8:10)]
names(st.test.df)
...
temp.df$Mean <- colMeans(s2.mRF.valid.df[,1:7])
temp.df$std  <- colSds(as.matrix(s2.mRF.valid.df[,1:7]))
temp.df
...
mRF.Block50 <- rfsrc(Multivar(N,Cab,Car,
                              Cw,Cm,LAI,hspot)~.,data=s2.mRF.train.df,block.size = 50,
                     #nsplit=0,
                     #samptype = "swor",
                     nodesize=1,
                     mtry=8)
...
#mRF.Block50.t <- rfsrc(Multivar(N,Cab,Car,
#                              Cw,Cm,LAI,hspot)~.,data=s2.mRF.train.df,block.size = 50)
...
mRF.temp.df <- data.frame(trait=names(valid.trait.df[,-c(8:10)]),
...
mRF.temp.df$Mean <- colMeans(s2.mRF.valid.df[,1:7])
mRF.temp.df$std  <- colSds(as.matrix(s2.mRF.valid.df[,1:7]))
mRF.temp.df
