...
param.maxmin <- matrix(c(10,60, #Cab
                         5,25, #Car
...
init.par.df <- data.frame(Cab=rtnorm(1,mean=40,lower=10,upper=60),
              Cw=rtnorm(1,mean=0.02,lower=0.01,upper=0.02),
              LAI=rtnorm(1,mean=1,sd=3,lower=0,upper=6),
              Cm= rtnorm(1,mean=0.02,lower=0.01,upper=0.02),
              Car=rtnorm(1,mean=1,sd=3,lower=5,upper=25)) #these are generated from random samples
...
prosail.runs <- 30
df.param.list <- data.frame(Cab=rtnorm(prosail.runs,mean=40,sd=15,lower=10,upper=60),
                            Cw=rtnorm(prosail.runs,mean=0.02,sd=0.005,lower=0.01,upper=0.02),
                            LAI=rtnorm(prosail.runs,mean=3.5,sd=2,lower=.5,upper=6),
                            Cm= rtnorm(prosail.runs,mean=0.02,lower=0.01,upper=0.02),
                            Car=rtnorm(prosail.runs,mean=1,sd=3,lower=5,upper=25))
...
write.csv(out.df.3,paste(dump.fld,"Optim_3Param_outdata.csv",sep="/"))
write.csv(out.df.4,paste(dump.fld,"Optim_4Param_outdata.csv",sep="/"))
write.csv(out.df.5,paste(dump.fld,"Optim_5Param_outdata.csv",sep="/"))
...
write.csv(sum.out.df,paste(dump.fld,"Optim_3Param_MAPE_Summary.csv",sep="/"))
...
#saving the plots
tiff(paste(dump.fld,"Optim5Params.tiff",sep="/"),
...
tiff(paste(dump.fld,"Optim4Params.tiff",sep="/"),
     units="px", width = 1024, height = 1024, res=124,
     compression = c("lzw"))
grid.arrange(cab.4.SCE, cab.4.LBF,
             Cw.4.SCE,  Cw.4.LBF,
...
tiff(paste(dump.fld,"Optim3Params.tiff",sep="/"),
     units="px", width = 1024, height = 1024, res=124,
     compression = c("lzw"))
