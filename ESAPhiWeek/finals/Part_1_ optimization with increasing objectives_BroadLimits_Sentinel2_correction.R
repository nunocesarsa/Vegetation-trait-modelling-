...
sam.fun.spclib.3 <- function(list.params,target.spectra=NULL){
  ...
  if (is.null(target.spectra)==F) {
    ...
    target.spectra <- spectralResampling(target.spectra,
                                         "Sentinel2",response_function = TRUE)[,c(2,3,4,5,6,7,9,12,13)]
...
write.csv(out.df.3,paste(dump.fld,"Optim_S2_3Param_outdata_BroadL_corrected.csv",sep="/"))
write.csv(out.df.4,paste(dump.fld,"Optim_S2_4Param_outdata_BroadL_corrected.csv",sep="/"))
write.csv(out.df.5,paste(dump.fld,"Optim_S2_5Param_outdata_BroadL_corrected.csv",sep="/"))
...
write.csv(sum.out.df,paste(dump.fld,"Optim_S2_3Param_MAPE_Summary_BroadL_corrected.csv",sep="/"))
...
#saving the plots
tiff(paste(dump.fld,"Optim_S2_5Params_BroadL_corrected.tif",sep="/"),
...
tiff(paste(dump.fld,"Optim_S2_4Params_BroadL_corrected.tif",sep="/"),
     units="px", width = 1024, height = 1024, res=124,
     compression = c("lzw"))
grid.arrange(cab.4.SCE, cab.4.LBF,
             Cw.4.SCE,  Cw.4.LBF,
...
tiff(paste(dump.fld,"Optim_S2_3Params_BroadL_corrected.tif",sep="/"),
     units="px", width = 1024, height = 1024, res=124,
     compression = c("lzw"))
