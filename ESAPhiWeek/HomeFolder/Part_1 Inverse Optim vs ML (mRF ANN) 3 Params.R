...
#for ANN - this package because it allows customization of the activation functions
#but.... has a limited number of options.
library(ANN2)
...
m.param.list.spectr <- spectra(df.param.list.spclib)
...
#lets train the ANN
Y.mat.300 <- as.matrix(df.300[,c(1:3)])
Y.mat.900 <- as.matrix(df.900[,c(1:3)])
Y.mat.1500 <- as.matrix(df.1500[,c(1:3)])

X.mat.300 <- as.matrix(df.300[,c(4:ncol(df.300))])
X.mat.900 <- as.matrix(df.900[,c(4:ncol(df.1500))])
X.mat.1500 <- as.matrix(df.1500[,c(4:ncol(df.1500))])



#an arbitary architecture: 1 layer of 6 nodes, 100 epochs
nn.300 <- neuralnetwork(X=X.mat.300,y=Y.mat.300,regression=T,hidden.layers =c(6),loss.type = "squared",
                        activ.functions = c('tanh'),n.epochs = 100,standardize = T)
nn.900 <- neuralnetwork(X=X.mat.900,y=Y.mat.900,regression=T,hidden.layers =c(6),loss.type = "squared",
                        activ.functions = c('tanh'),n.epochs = 100,standardize = T)
nn.1500 <- neuralnetwork(X=X.mat.1500,y=Y.mat.1500,regression=T,hidden.layers =c(6),loss.type = "squared",
                        activ.functions = c('tanh'),n.epochs = 100,standardize = T)

#once this is done, we can create a df with everything
...
#storing the outputs of random forest
...
#storing the outputs of ANN
out.df$ann_Cab300 <- mv.mRF.pred.300[,1]
out.df$ann_Cw300  <- mv.mRF.pred.300[,2]
out.df$ann_LAI300 <- mv.mRF.pred.300[,3]
out.df$ann_Cab900 <- mv.mRF.pred.900[,1]
out.df$ann_Cw900  <- mv.mRF.pred.900[,2]
out.df$ann_LAI900 <- mv.mRF.pred.900[,3]
out.df$ann_Cab1500 <- mv.mRF.pred.1500[,1]
out.df$ann_Cw1500  <- mv.mRF.pred.1500[,2]
out.df$ann_LAI1500 <- mv.mRF.pred.1500[,3]

head(out.df)


#lets bring it all into a plot
par(mfrow=c(2,4))
plot(out.df$Cab,out.df$Cab,pch=19,xlab="Reference",ylab="Prediction",main="Cab",cex=1.5)
points(out.df$Cab,out.df$SCE_Cab,pch=2,col="blue",cex=.75)
points(out.df$Cab,out.df$LBFGSB_Cab,pch=3,col="red",cex=.75)
...
#COMPARING THE DIFFERENT ML MODELS

par(mfrow=c(3,2))
plot(out.df$Cab,out.df$Cab,pch=19,xlab="Reference",ylab="Prediction",main="Cab mRF",cex=1.5)
points(out.df$Cab,out.df$mRF_Cab300,pch=13,col="blue",cex=.75)
points(out.df$Cab,out.df$mRF_Cab900,pch=15,col="red",cex=.75)
points(out.df$Cab,out.df$mRF_Cab1500,pch=23,col="green",cex=.75)
...
plot(out.df$Cab,out.df$Cab,pch=19,xlab="Reference",ylab="Prediction",main="Cab ANN",cex=1.5)
points(out.df$Cab,out.df$ann_Cab300,pch=13,col="blue",cex=.75)
points(out.df$Cab,out.df$ann_Cab900,pch=15,col="red",cex=.75)
points(out.df$Cab,out.df$ann_Cab1500,pch=23,col="green",cex=.75)

abline(lm(out.df$ann_Cab300~out.df$Cab),col="blue")
abline(lm(out.df$ann_Cab900~out.df$Cab),col="red")
abline(lm(out.df$ann_Cab1500~out.df$Cab),col="green")


plot(out.df$Cw,out.df$Cw,pch=19,xlab="Reference",ylab="Prediction",main="Cw mRF",cex=1.5)
points(out.df$Cw,out.df$mRF_Cw300,pch=13,col="blue",cex=.75)
points(out.df$Cw,out.df$mRF_Cw900,pch=15,col="red",cex=.75)
points(out.df$Cw,out.df$mRF_Cw1500,pch=23,col="green",cex=.75)
abline(lm(out.df$mRF_Cw300~out.df$Cw),col="blue")
...
plot(out.df$Cw,out.df$Cw,pch=19,xlab="Reference",ylab="Prediction",main="Cw ANN",cex=1.5)

points(out.df$Cw,out.df$ann_Cw300,pch=13,col="blue",cex=.75)
points(out.df$Cw,out.df$ann_Cw900,pch=15,col="red",cex=.75)
points(out.df$Cw,out.df$ann_Cw1500,pch=23,col="green",cex=.75)

abline(lm(out.df$ann_Cw300~out.df$Cw),col="blue")
abline(lm(out.df$ann_Cw900~out.df$Cw),col="red")
abline(lm(out.df$ann_Cw1500~out.df$Cw),col="green")


plot(out.df$LAI,out.df$SCE_LAI,pch=19,xlab="Reference",ylab="Prediction",main="LAI mRF",cex=1.5)
points(out.df$LAI,out.df$mRF_LAI300,pch=13,col="blue",cex=.75)
points(out.df$LAI,out.df$mRF_LAI900,pch=15,col="red",cex=.75)
points(out.df$LAI,out.df$mRF_LAI1500,pch=23,col="green",cex=.75)
...
plot(out.df$LAI,out.df$SCE_LAI,pch=19,xlab="Reference",ylab="Prediction",main="LAI ANN",cex=1.5)

points(out.df$LAI,out.df$ann_LAI300,pch=13,col="blue",cex=.75)
points(out.df$LAI,out.df$ann_LAI900,pch=15,col="red",cex=.75)
points(out.df$LAI,out.df$ann_LAI1500,pch=23,col="green",cex=.75)

abline(lm(out.df$ann_LAI300~out.df$LAI),col="blue")
abline(lm(out.df$ann_LAI900~out.df$LAI),col="red")
abline(lm(out.df$ann_LAI1500~out.df$LAI),col="green")


#lets dump the predictions
write.csv(out.df,paste(dump.fld,
                       "Inverse_optimization_3Params_SCEvsLBFGSBvsMRFvsANN.csv",sep="/"))
